...
* [x] **Polyglot AST Analysis:** `get_diagnostic_report` detects performance traps across Java (REST Assured client churn, missing timeouts, repeated schema reloads — `src/review/rules.rs`) and Python (`time.sleep`, blocking calls in `async def test_*`, unclosed client sessions, broad fixture scopes — `src/feedback.rs`). Covered by `tests/review_tests.rs`.
* [x] **OTel Hinting:** `get_hints` provides progressive 3-tier guidance (Architectural Nudge, API Pattern, Code Diff) for telemetry topics via `ProgressiveHints::telemetry_hints()` (`src/mcp.rs`, `src/feedback.rs`), covered by `tests/mcp_server_tests.rs`. The Crucible backend now genuinely records and serves spans it can hint about — see the next item's caveat.
* [ ] **Micro-Crucible Expansion:** `docker-compose.yml` and `otel-collector-config.yaml` stand up real `kafka` and `otel-collector` containers, and the `backend` service is configured with `KAFKA_BOOTSTRAP_SERVERS` / `OTEL_EXPORTER_OTLP_ENDPOINT` — but nothing in `crucible/backend/*.py` reads either variable, and neither a Kafka client nor the `opentelemetry` SDK is a dependency (`requirements.txt`). The containers run; the backend never talks to them. What *is* real and useful for the curriculum: `/transfer`'s `kafka_lag` is a simulated delay via the existing chaos-injection layer (not a Kafka producer/consumer), and `crucible/backend/tracing.py` records real W3C `traceparent`-correlated spans queryable via `GET /api/telemetry/spans` — genuine trace-context propagation, just not exported as OTLP to the collector container sitting next to it. Marking this item complete would overstate what a learner can actually observe by pointing a real OTel Collector or Kafka consumer at this stack.
* [x] **Badging System:** "Chaos Survivor" and "The Architect" (`BadgeCard.tsx`, `BadgesShowcase.tsx`) render from live `/api/progress` data, not static completion props. `e2e/16-badges.spec.ts`'s selectors (`data-testid`, `data-unlocked`) all correspond to the real component markup.
